...
description: |
  Tutorial de como configurar o Git no Rstudio.
categories:
  - Rstudio
  - Git
  - GitHub
...
Esta configuração é algo que é feito apenas uma única vez na vida (ok não na vida, mas uma única vez por computador) e este turorial vai te guiar no passo a passo.
...
Enseguida escolha a opção `Git` e preencha a opção `Repository URL` com endereço que acabou de copiar do GitHub. Escolha também onde quer criar o projeto no seu computador (no meu caso `/Volumes/CTRUCIOS_SD/Research`). Não esqueça marcar a opção `Open in new session`.
...
**Pronto!** Agora seu repositório no GitHub e seu novo projeto do Rstudio estão vinculados, agora é só escrever seus códigos e fazer commits, Push and Pulls.
